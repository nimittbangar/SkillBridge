      ...
      <div className="d-flex">
        ...
        <div className="flex-fill main-content p-3">
          ...
          <div className="welcome-header mb-4 d-flex justify-content-between align-items-start flex-wrap gap-2">
            ...
            {/* Download CSV — React-controlled dropdown (no reliance on Bootstrap JS) */}
            <div className="dropdown" style={{ position:'relative' }} ref={menuRef}>
              <button className="btn btn-light btn-sm rounded-pill fw-semibold px-3"
                type="button"
              ...
              {menuOpen && (
                <ul className="dropdown-menu dropdown-menu-end shadow-sm rounded-3 show"
                  style={{ display:'block', position:'absolute', right:0, top:'100%', marginTop:6, zIndex:1000, minWidth:230 }}>
                  <li><h6 className="dropdown-header">Export applicants by stage</h6></li>
                  {EXPORT_OPTIONS.map((opt) => (
                    <li key={opt.status}>
                      <button className="dropdown-item d-flex align-items-center gap-2"
                        disabled={exporting === opt.status}
                        onClick={() => { handleExport(opt); setMenuOpen(false) }}>
